    ...
    if skeletal_mesh:
        ...
        if mannequin:
            ...
            if False:  # Disabled to show only one cube
                ...
                base_mat = unreal.load_object(None, "/Engine/BasicShapes/BasicShapeMaterial.BasicShapeMaterial")
                ...
                log(f"⚠ Warning: Failed to create colored plus sign: {e}")
            ...
            if False:  # Disabled to show only one cube
                ...
                if cube_mesh:
                    ...
                    if focus_actor:
                        ...
                        if base_mat:
                            try:
                                mid = unreal.KismetMaterialLibrary.create_dynamic_material_instance(focus_actor, base_mat)
                                mid.set_vector_parameter_value("Color", unreal.LinearColor(1.0, 0.5, 0.0, 1.0))  # orange
                                smc.set_material(0, mid)
                ...
                log(f"⚠ Warning: Could not add origin flat cube: {e}")
            ...
            try:
                ...
                test_base_mat = unreal.load_object(None, "/Engine/BasicShapes/BasicShapeMaterial.BasicShapeMaterial")
                if test_cube_mesh:
                    ...
                    if test_cube:
                        ...
                        if test_base_mat:
                            try:
                                mid = unreal.KismetMaterialLibrary.create_dynamic_material_instance(test_cube, test_base_mat)
                                mid.set_vector_parameter_value("Color", unreal.LinearColor(0.0, 0.0, 1.0, 1.0))
                                smc.set_material(0, mid)
                                log("✓ Test big blue cube spawned at origin")
                            except Exception as e:
                                smc.set_material(0, test_base_mat)
                                log(f"⚠ Test cube material applied without color param: {e}")
                        else:
                            log("⚠ Test cube: base material not found; using default")
                    else:
            ...
            try:
                ...
                if delta_cm <= 0.5:
                    log(f"✓ Camera position verified (≤0.5cm): expected=({cam_loc.x:.1f}, {cam_loc.y:.1f}, {cam_loc.z:.1f}), actual=({actual_loc.x:.1f}, {actual_loc.y:.1f}, {actual_loc.z:.1f})")
                else:
                    log(f"⚠ Camera position mismatch by {delta_cm:.2f}cm\n   expected=({cam_loc.x:.1f}, {cam_loc.y:.1f}, {cam_loc.z:.1f})\n   actual=({actual_loc.x:.1f}, {actual_loc.y:.1f}, {actual_loc.z:.1f})")
