...
# Step 6: Verify root component
print("[STEP 6] Verifying root component...")
# This announces we're checking the root component setup
# SkeletalMeshActor automatically sets the skeletal mesh component as root
# We verify this to understand the actor's hierarchy

root = character.get_root_component()
# This retrieves the current root component from the actor
# For SkeletalMeshActor, this is already set to the skeletal mesh component
# No need to manually set it like we would with a base Actor

print(f"✓ Root component: {root.get_name()}")
# This shows which component is the root
# Should be the skeletal mesh component we retrieved earlier
# The root component determines the actor's overall transform

print(f"  Is skeletal mesh?: {root == mesh_comp}\n")
# This checks if the root is the skeletal mesh component
# Should print "True" for SkeletalMeshActor
# This confirms the component hierarchy is set up correctly
...
sk_mesh_path = "/Game/Characters/Mannequins/Meshes/SKM_Quinn"
# This defines the path to the skeletal mesh asset in your project
# All content paths start with /Game/ which represents your Content folder
# You need to replace this with the actual path to your skeletal mesh
...
# The component now knows what 3D model to render
...
anim_bp_path = "/Game/Characters/Mannequins/Animations/ABP_Quinn"
# This defines the path to the Animation Blueprint in your project
# Animation Blueprints usually start with "ABP_" by convention
# Replace this with your actual Animation Blueprint path
...
# This retrieves the currently assigned animation class from the component
# We use this to verify the assignment worked
# Should return the same class we just assigned

print(f"✓ Animation blueprint class assigned: {anim_class.get_name() if anim_class else 'None'}")
# This confirms which animation class is now assigned
# Should show the name of your Animation Blueprint's class
# If it says "None", the assignment failed
...
print(f"Root Component:    {character.get_root_component().get_name()}")
# This shows which component is serving as the root
# Should display the skeletal mesh component's name
# Verifies the root was set correctly
